...
public class Interpreter {
	...
	private void invokeLibProcedure(Procedure procedure, Stack stack) {
		try {
			if(procedure.name.equals("read_int")){
				BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
				String line = in.readLine();
				stack.push(new Integer(line));
			} else if(procedure.name.equals("read_float")){
				BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
				String line = in.readLine();
				stack.push(new Float(line));
			} else if(procedure.name.equals("read_char")){
				BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
				String line = in.readLine();
				if(line.length()!=1){
					throw new RuntimeException("read_char can only read line of length 1");
				}
				stack.push(new Integer(line.toCharArray()[0]));
			} else if(procedure.name.equals("read_string")){
				BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
				String line = in.readLine();
				stack.push(line.split(" ")[0]);
			} else if(procedure.name.equals("read_line")){
				BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
				String line = in.readLine();
				stack.push(line);
			} else if (procedure.name.equals("print_int")){
				Integer intValue = (Integer) stack.pop();
				System.out.print(intValue);
			} else if(procedure.name.equals("print_float")){
			...
			} else if (procedure.name.equals("print_str")){
				String stringValue = (String) stack.pop();
				System.out.print(stringValue);
			} else if (procedure.name.equals("print_line")){
				String stringValue = (String) stack.pop();
